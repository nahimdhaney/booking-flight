...
import { PaymentFactoryService, PaymentServices } from 'src/application/useCases/payment';
...
describe('PaymentUseCases Test', () => {
  ...
  it('shoud return an empty list of Payments', async () => {
    ...
    jest.spyOn(paymentServices, 'getAllPayments').mockImplementation(async () => []);
  ...
  it('Should return an list of Payments with one', async () => {
    ...
    const payment = new Payment(new TransactionNumber(12345),'booking-id', new Amount(123))

    jest.spyOn(paymentServices, 'getAllPayments').mockImplementation(async () => [payment]);
